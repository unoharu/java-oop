...
### [InsufficientFundsException.java](../src/main/java/oop/step14/InsufficientFundsException.java)

Checked 例外（`extends Exception`）。残高と要求額をフィールドに保持します。`throws` 宣言が必須です。

### [NegativeAmountException.java](../src/main/java/oop/step14/NegativeAmountException.java)

Unchecked 例外（`extends RuntimeException`）。不正な引数を表します。`throws` 宣言は不要です。

### [BankAccount.java](../src/main/java/oop/step14/BankAccount.java)

`deposit()` は `NegativeAmountException`（unchecked）を投げます。`withdraw()` は `InsufficientFundsException`（checked）を `throws` 宣言して投げます。

### [ResourceDemo.java](../src/main/java/oop/step14/ResourceDemo.java)
...
```bash
mvn compile exec:java -pl step14_exception_handling
```
...
次のステップ → [Step 15 - Singleton パターン](../../step15_pattern_singleton/docs/article.md)
