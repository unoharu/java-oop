...
### [Button.java](../src/main/java/oop/step12/Button.java)

`render()` と `onClick()` を持つインタフェース。呼び出し側はこの型だけを使います。

### [WindowsButton.java](../src/main/java/oop/step12/WindowsButton.java) / [MacButton.java](../src/main/java/oop/step12/MacButton.java) / [LinuxButton.java](../src/main/java/oop/step12/LinuxButton.java)

`Button` を実装する具体クラス。OS ごとに異なる実装を持ちます。

### [ButtonFactory.java](../src/main/java/oop/step12/ButtonFactory.java)

`switch` 式でOS文字列から適切な `Button` を生成します。未知のOSには `IllegalArgumentException` を投げます（step09 の接続）。
...
```bash
mvn compile exec:java -pl step12_pattern_factory
```
...
次のステップ → [Step 13 - Observer パターン](../../step13_pattern_observer/docs/article.md)
